...
(function($) {
    ...
    $(document).ready(function() {
        ...
        var SPRITES_MEDIUM_5500x5500 = {
            ...
            TREE1:                  { x:  4046, y:   883, w:  1404, h:  1408 },
        ...
        function play_modals() {
            ...
            function delete_all_jse_cookies() {
                // Delete all Cookies
        ...
        function gameTiltHorizonFx() {
            ...
            var tiltHorizonFxSaveOnce          = 0;
            var tiltHorizonFxLastStatus        = null;
            
            // Run by Game > Tilt Horizon Fx.
            jsr.HelperRunTiltHorizonFx = function( isActive, baseSegmentCurve, currentSpeed, maxSpeed, id, context, canvasWidth, canvasHeight) {
                
                // Save the status of the canvas before applying effects.
                if( tiltHorizonFxSaveOnce == 0 ) {
                    tiltHorizonFxSaveOnce = 1;
                    context.save();
                }
                
                // Reset to initial conditions.
                function resetTiltHorizonFxEngine( id, canvasWidth, canvasHeight) {
                    // Reset Tilt Fx applied by Context.
                    context.restore();
                    // Reset Tilt Fx applied to the canvas by css.
                ...
                // Memorizza tipo Fx richiesto
                if( tiltHorizonFxResetOnce == 0 ) {
                    tiltHorizonFxResetOnce = 1;
                    tiltHorizonFxLastStatus = jsr.Vars.tiltHorizonFxIsActive;
                }
                // Se è cambiato esegui il reset.
                if( tiltHorizonFxLastStatus != jsr.Vars.tiltHorizonFxIsActive ) {
                    tiltHorizonFxResetOnce = 0;
                    resetTiltHorizonFxEngine( id, canvasWidth, canvasHeight);
                }
                // Run Tilt Orizon Fx.
                function runTiltHorizonFxEngine( tiltEngine, tiltHorizonFxRotation, id, canvasWidth, canvasHeight) {
                    // Perform x axis rotation by tiltHorizonFxRotation.
                    switch( tiltEngine ) {
                        // Fx by Drowing Object.
                        case 'drawing':
                            context.translate(  canvasWidth / 2,  canvasHeight / 2);
                            context.rotate( -tiltHorizonFxRotation * (Math.PI/90) );
                            context.translate( -canvasWidth / 2, -canvasHeight / 2);
                ...
                if ( isActive != 'disabled' ) {
                    ...
                    runTiltHorizonFxEngine( isActive, tiltHorizonFxRotation, id, canvasWidth, canvasHeight);
                }
                
            }
            
            // Change Weather Button
            $(document).on('change','.settings.modal #currentTiltHorizonFx',function(){
                jsr.Vars.tiltHorizonFxIsActive = $('.settings.modal #currentTiltHorizonFx option:selected').val();
            });
            
            // Set Default Tilt Horizon Fx value for Select.
            jsr.HelperSetTiltHorizonFxForSelect = function() {
                $('.settings.modal #currentTiltHorizonFx').val(jsr.Vars.tiltHorizonFxIsActive);
            }
