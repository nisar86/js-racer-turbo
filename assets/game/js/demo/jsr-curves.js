...
(function($) {
    ...
    function racer() {
        ...
        var RenderGame = function() {
          ...
          jsr.Render.background(ctx, background, width, height, jsr.BACKGROUND.SKY,   skyOffset);
          jsr.Render.background(ctx, background, width, height, jsr.BACKGROUND.HILLS, hillOffset);
          jsr.Render.background(ctx, background, width, height, jsr.BACKGROUND.TREES, treeOffset);
          ...
          for(n = 0 ; n < drawDistance ; n++) {
            ...
            jsr.Util.project(segment.p1, (playerX * roadWidth) - x,      cameraHeight, position - (segment.looped ? trackLength : 0), cameraDepth, width, height, roadWidth);
            jsr.Util.project(segment.p2, (playerX * roadWidth) - x - dx, cameraHeight, position - (segment.looped ? trackLength : 0), cameraDepth, width, height, roadWidth);
        ...
        function addRoad(enter, hold, leave, curve) {
            var n;
            for(n = 0 ; n < enter ; n++)
                addSegment(jsr.Util.easeIn(0, curve, n/enter));
            for(n = 0 ; n < hold  ; n++)
                addSegment(curve);
            for(n = 0 ; n < leave ; n++)
                addSegment(jsr.Util.easeInOut(curve, 0, n/leave));
        }
